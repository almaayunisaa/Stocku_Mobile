...
class TileProdukLaris extends StatelessWidget {
  ...
  TileProdukLaris({super.key});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      if (laporanController.produkLaris.isEmpty) {
        return Padding(
      ...
      return ListView.builder(
        itemCount: laporanController.produkLaris.length,
        shrinkWrap: true,
        physics: NeverScrollableScrollPhysics(),
        itemBuilder: (context, index) {
          final produk = laporanController.produkLaris[index];
          final int total = laporanController.produkLaris.length - 1;
          final double factor = (total - index) / total;
